...
- Source version: `gh-run-receptor@0.21.1`
...
Version `0.21.1` is a source preview with:
...
are not implemented in `0.21.1`. Private-repository and fork token behavior remains
...
jobs:
  report:
    ...
    steps:
      - uses: uibcdf/gh-run-receptor@0.21.1
        with:
...
jobs:
  report:
    uses: uibcdf/gh-run-receptor/.github/workflows/reusable-report.yml@0.21.1
    with:
      run-id: ${{ github.event.workflow_run.id }}
      repository: ${{ github.repository }}
      profile: ci
...
without `$/` are not claimed. Pin the full 0.21.1 commit instead of the tag when an
...
Version 0.21.1 consumes attempt-qualified `events@1` artifacts emitted by a producer and
...
Version `0.21.1` supports exactly one identity per rule: an exact `path`, positive numeric
